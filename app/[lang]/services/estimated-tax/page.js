...
export async function generateMetadata({ params }) {
    ...
    return buildPageMetadata({
        lang,
        path: '/services/estimated-tax/',
        title: dict.estimatedTaxPage.title,
        description: dict.estimatedTaxPage.subtitle
    });
}

export default async function EstimatedTaxServices({ params }) {
    const { lang } = await params;
    const dict = await getDictionary(lang);
    const t = dict.estimatedTaxPage;
                    ...
                    <div className="service-content" style={{ maxWidth: '800px', margin: '0 auto' }}>
                        ...
                        <div className="service-grid mt-8">
                            <div className="service-card-small">
                                <h3>{t.cards.calc}</h3>
                                <p>{t.cards.calcDesc}</p>
                            </div>
                            <div className="service-card-small">
                                <h3>{t.cards.cashflow}</h3>
                                <p>{t.cards.cashflowDesc}</p>
                            </div>
                            <div className="service-card-small">
                                <h3>{t.cards.tracking}</h3>
                                <p>{t.cards.trackingDesc}</p>
                            </div>
                            <div className="service-card-small">
                                <h3>{t.cards.scheduling}</h3>
                                <p>{t.cards.schedulingDesc}</p>
                            </div>
                        </div>
